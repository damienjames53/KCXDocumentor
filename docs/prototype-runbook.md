...
- Claude Sonnet 4.6 can generate `guide_draft.json` through the authenticated Azure Function proxy when `KCXDOC_REMOTE_API_BASE_URL` is configured locally and `KCXDOC_FOUNDRY_API_KEY` is configured on the Function App.
